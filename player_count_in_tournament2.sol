...
contract PlayerCount is ChainlinkClient {
     ...
     */ 
    function fulfill(bytes32 _requestId, uint data,uint data2, uint data3) public recordChainlinkFulfillment(_requestId)
    {
        batCount = data;
        bowlCount = data2;
    ...
    function createPlayerCountRequest(string memory url,string memory _jobId,address _oracle) public returns(uint) {
        // string memory empty = "";
        // if(keccak256(bytes(empty)) == keccak256(bytes(playerId)))
        // {
        uint requestId = uint(submitPlayerCountRequest(url,_jobId,_oracle));
        return requestId;
        // }
        // else
        // {
    ...
    function getRequestResult2() public view returns (uint) {
        return bowlCount;
    }
    
    function getRequestResult3() public view returns (uint) {
